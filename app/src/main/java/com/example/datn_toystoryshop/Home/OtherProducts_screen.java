...
public class OtherProducts_screen extends AppCompatActivity {
        ...
        apiService.getOther().enqueue(new Callback<List<Product_Model>>() {
            ...
            public void onResponse(Call<List<Product_Model>> call, Response<List<Product_Model>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    adapter = new Product_No_Star_Adapter(OtherProducts_screen.this, originalProductList, documentId);
    ...
    private void showSortDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(OtherProducts_screen.this);
        String[] sortOptions = {
                "Sắp xếp theo A-Z",
                "Sắp xếp theo Z-A",
                "Giá từ thấp đến cao",
    ...
    private void sortProducts(String sortBy) {
        if (originalProductList == null || originalProductList.isEmpty()) {
            Toast.makeText(OtherProducts_screen.this, "Không có sản phẩm để sắp xếp.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        btnApplyFilter.setOnClickListener(v -> {
            ...
            if (!(isBrand1Selected || isBrand2Selected || isBrand3Selected || isBrand4Selected || isBrand5Selected || isBrand6Selected)) {
                ...
                if (minPrice != 0 || maxPrice != 0) {
                    ...
                    Toast.makeText(OtherProducts_screen.this, "Vui lòng chọn ít nhất một thương hiệu hoặc một khoảng giá!", Toast.LENGTH_SHORT).show();
                }
            } else {
