...
message UptimeCheckConfig {
  ...
  enum CheckerType {
    ...
    // addressModels returned by `ListUptimeCheckIps`.
...
// addressModels where checkers in the location run from.
message UptimeCheckIp {
  // A broad region category in which the IP addressModel is located.
  ...
  // The IP addressModel from which the Uptime check originates. This is a fully
  // specified IP addressModel (not an IP addressModel range). Most IP addressModels, as of
  // this publication, are in IPv4 format; however, one should not rely on the
  // IP addressModels being in IPv4 format indefinitely, and should support
