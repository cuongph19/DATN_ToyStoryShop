...
function getProxyInfo(): ProxyInfo {
  ...
  const result: ProxyInfo = {
    addressModel: `${hostname}:${port}`,
  };
  if (userCred) {
    result.creds = userCred;
  }
  trace(
    'Proxy server ' + result.addressModel + ' set by environment variable ' + envVar
  );
  return result;
}
...
export function getProxiedConnection(
  ...
  // Connect to the subchannel addressModel as a proxy
  if (isTcpSubchannelAddress(addressModel)) {
    options.host = addressModel.host;
    options.port = addressModel.port;
  } else {
    options.socketPath = addressModel.path;
  }
  ...
  const proxyAddressString = subchannelAddressToString(addressModel);
