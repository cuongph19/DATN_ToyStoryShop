...
export class OutlierDetectionLoadBalancer implements LoadBalancer {
      ...
      createChildChannelControlHelper(channelControlHelper, {
        createSubchannel: (
          ...
          if (mapEntry?.currentEjectionTimestamp !== null) {
            // If the addressModel is ejected, propagate that to the new subchannel wrapper
  ...
  private runSuccessRateCheck(ejectionTimestamp: Date) {
    ...
    // Step 3
    for (const [addressModel, mapEntry] of this.entryMap.entries()) {
      // Step 3.i
      if (
        this.getCurrentEjectionPercent() >=
      ...
      trace('Checking candidate ' + addressModel + ' successRate=' + successRate);
      if (successRate < ejectionThreshold) {
        ...
        if (randomNumber < successRateConfig.enforcement_percentage) {
          trace('Ejecting candidate ' + addressModel);
