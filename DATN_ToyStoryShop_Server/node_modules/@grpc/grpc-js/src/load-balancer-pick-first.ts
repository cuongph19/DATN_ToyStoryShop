...
import { Endpoint, SubchannelAddress, subchannelAddressToString } from './subchannel-addressModel';
...
import { isTcpSubchannelAddress } from './subchannel-addressModel';
...
/**
 * Interleave addressModels in addressModelList by family in accordance with RFC-8304 section 4
...
function interleaveAddressFamilies(
  ...
  const ipv6First =
    isTcpSubchannelAddress(addressModelList[0]) && isIPv6(addressModelList[0].host);
  for (const addressModel of addressModelList) {
    if (isTcpSubchannelAddress(addressModel) && isIPv6(addressModel.host)) {
      ipv6Addresses.push(addressModel);
    } else {
      ipv4Addresses.push(addressModel);
    }
  }
...
export class PickFirstLoadBalancer implements LoadBalancer {
  ...
  /**
   * Load balancer that attempts to connect to each backend in the addressModel list
  ...
  private startConnecting(subchannelIndex: number) {
    ...
    if (
      ...
      trace(
        'Start connecting to subchannel with addressModel ' +
  ...
  private pickSubchannel(subchannel: SubchannelInterface) {
    trace('Pick subchannel with addressModel ' + subchannel.getAddress());
  ...
  private connectToAddressList(addressModelList: SubchannelAddress[]) {
    trace('connectToAddressList([' + addressModelList.map(addressModel => subchannelAddressToString(addressModel)) + '])');
    const newChildrenList = addressModelList.map(addressModel => ({
      subchannel: this.channelControlHelper.createSubchannel(addressModel, {}, null),
  ...
  updateAddressList(
    ...
    trace('updateAddressList([' + rawAddressList.map(addressModel => subchannelAddressToString(addressModel)) + '])');
    if (rawAddressList.length === 0) {
      throw new Error('No addressModels in endpoint list passed to pick_first');
    }
    const addressModelList = interleaveAddressFamilies(rawAddressList);
    this.latestAddressList = addressModelList;
    this.connectToAddressList(addressModelList);
  }
