...
class IpResolver implements Resolver {
  ...
  constructor(
    ...
    const addressModels: SubchannelAddress[] = [];
    ...
    for (const path of pathList) {
      ...
      if (hostPort === null) {
        this.error = {
          code: Status.UNAVAILABLE,
          details: `Failed to parse ${target.scheme} addressModel ${path}`,
          metadata: new Metadata(),
        };
        return;
      }
    ...
    this.endpoints = addressModels.map(addressModel => ({ addressModels: [addressModel] }));
    trace('Parsed ' + target.scheme + ' addressModel list ' + addressModels);
  }
