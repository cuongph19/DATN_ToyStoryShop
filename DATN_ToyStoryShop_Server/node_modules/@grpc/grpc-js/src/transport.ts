...
class Http2Transport implements Transport {
  ...
  constructor(
     ...
     * addressModel, i.e. if connecting through an HTTP CONNECT proxy.
     */
...
export class Http2SubchannelConnector implements SubchannelConnector {
  ...
  private createSession(
    ...
    return new Promise<Http2Transport>((resolve, reject) => {
      ...
      if (proxyConnectionResult.realTarget) {
        ...
        remoteName = null;
        this.trace(
          'creating HTTP/2 session to ' + subchannelAddressToString(addressModel)
        );
      }
      ...
      if ('secureContext' in connectionOptions) {
        ...
        connectionOptions.createConnection = (authority, option) => {
          if (proxyConnectionResult.socket) {
            ...
            return net.connect(addressModel);
          }
        };
      }
      ...
      session.once('connect', () => {
        ...
        resolve(new Http2Transport(session, addressModel, options, remoteName));
        this.session = null;
      });
      session.once('close', () => {
        this.session = null;
  ...
  connect(
    ...
    return getProxiedConnection(addressModel, options, connectionOptions).then(
      result => this.createSession(addressModel, credentials, options, result)
    );
  }
