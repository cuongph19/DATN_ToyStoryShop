...
/**
 * This represents a single backend addressModel to connect to. This interface is a
...
): addressModel is TcpSubchannelAddress {
  return 'port' in addressModel;
}
...
export function subchannelAddressToString(addressModel: SubchannelAddress): string {
  if (isTcpSubchannelAddress(addressModel)) {
    if (isIPv6(addressModel.host)) {
      return '[' + addressModel.host + ']:' + addressModel.port;
    } else {
      return addressModel.host + ':' + addressModel.port;
    }
  } else {
    return addressModel.path;
  }
}
...
export interface Endpoint {
  addressModels: SubchannelAddress[];
}

export function endpointEqual(endpoint1: Endpoint, endpoint2: Endpoint) {
  if (endpoint1.addressModels.length !== endpoint2.addressModels.length) {
    return false;
  }
  for (let i = 0; i < endpoint1.addressModels.length; i++) {
    if (
      !subchannelAddressEqual(endpoint1.addressModels[i], endpoint2.addressModels[i])
    ) {
      return false;
    }
  }
...
export function endpointToString(endpoint: Endpoint): string {
  return (
    '[' + endpoint.addressModels.map(subchannelAddressToString).join(', ') + ']'
  );
}
...
export function endpointHasAddress(
  ...
  for (const addressModel of endpoint.addressModels) {
    if (subchannelAddressEqual(addressModel, expectedAddress)) {
      return true;
    }
  }
  return false;
...
function endpointEqualUnordered(
  ...
  for (const address1 of endpoint1.addressModels) {
    let matchFound = false;
    for (const address2 of endpoint2.addressModels) {
...
export class EndpointMap<ValueType> {
  ...
  getForSubchannelAddress(addressModel: SubchannelAddress): ValueType | undefined {
    for (const entry of this.map) {
      if (endpointHasAddress(entry.key, addressModel)) {
        return entry.value;
      }
    }
    return undefined;
