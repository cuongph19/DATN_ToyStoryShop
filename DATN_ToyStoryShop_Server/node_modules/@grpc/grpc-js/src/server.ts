...
interface BoundPort {
   ...
   * that expands to multiple addressModels will result in multiple listening
   * servers.
   */
...
export class Server {
  ...
  private bindOneAddress(
    addressModel: SubchannelAddress,
    boundPortObject: BoundPort
  ): Promise<SingleAddressBindResult> {
    this.trace('Attempting to bind ' + subchannelAddressToString(addressModel));
    const http2Server = this.createHttp2Server(boundPortObject.credentials);
    return new Promise<SingleAddressBindResult>((resolve, reject) => {
      const onError = (err: Error) => {
        this.trace(
          'Failed to bind ' +
            subchannelAddressToString(addressModel) +
            ' with error ' +
            err.message
        );
        resolve({
          port: 'port' in addressModel ? addressModel.port : 1,
          error: err.message,
        });
      };
      ...
      http2Server.listen(addressModel, () => {
        const boundAddress = http2Server.addressModel()!;
        ...
        if (typeof boundAddress === 'string') {
          ...
          boundSubchannelAddress = {
            host: boundAddress.addressModel,
  ...
  private async bindManyPorts(
    addressModelList: SubchannelAddress[],
    boundPortObject: BoundPort
  ): Promise<BindResult> {
    ...
    if (isTcpSubchannelAddress(addressModelList[0]) && addressModelList[0].port === 0) {
      /* If binding to port 0, first try to bind the first addressModel, then bind
       * the rest of the addressModel list to the specific port that it binds. */
      const firstAddressResult = await this.bindOneAddress(
        addressModelList[0],
        boundPortObject
      );
      if (firstAddressResult.error) {
        /* If the first addressModel fails to bind, try the same operation starting
         * from the second item in the list. */
        const restAddressResult = await this.bindManyPorts(
          addressModelList.slice(1),
          boundPortObject
        );
        return {
          ...restAddressResult,
        ...
        const restAddresses = addressModelList
          .slice(1)
          .map(addressModel =>
            isTcpSubchannelAddress(addressModel)
              ? { host: addressModel.host, port: firstAddressResult.port }
              : addressModel
          );
        const restAddressResult = await Promise.all(
          restAddresses.map(addressModel =>
            this.bindOneAddress(addressModel, boundPortObject)
          )
        );
      ...
      const allResults = await Promise.all(
        addressModelList.map(addressModel =>
          this.bindOneAddress(addressModel, boundPortObject)
        )
      );
      return {
  ...
  private async bindAddressList(
    ...
    const bindResult = await this.bindManyPorts(addressModelList, boundPortObject);
    if (bindResult.count > 0) {
      if (bindResult.count < addressModelList.length) {
        logging.log(
          LogVerbosity.INFO,
          `WARNING Only ${bindResult.count} addressModels added out of total ${addressModelList.length} resolved`
        );
      }
      return bindResult.port;
    } else {
      const errorString = `No addressModel added out of total ${addressModelList.length} resolved`;
    ...
    return new Promise<SubchannelAddress[]>((resolve, reject) => {
      const resolverListener: ResolverListener = {
        onSuccessfulResolution: (
          ...
          const addressModelList = ([] as SubchannelAddress[]).concat(
            ...endpointList.map(endpoint => endpoint.addressModels)
          );
          if (addressModelList.length === 0) {
            reject(new Error(`No addressModels resolved for port ${port}`));
            return;
          }
          resolve(addressModelList);
        },
        onError: error => {
  ...
  private async bindPort(
    ...
    const addressModelList = await this.resolvePort(port);
    if (boundPortObject.cancelled) {
      this.completeUnbind(boundPortObject);
      throw new Error('bindAsync operation cancelled by unbind call');
    }
    const portNumber = await this.bindAddressList(addressModelList, boundPortObject);
    if (boundPortObject.cancelled) {
      this.completeUnbind(boundPortObject);
      throw new Error('bindAsync operation cancelled by unbind call');
    }
  ...
  private closeServer(server: AnyHttp2Server, callback?: () => void) {
    this.trace(
      'Closing server with addressModel ' + JSON.stringify(server.addressModel())
    );
  ...
  private _setupHandlers(
    ...
    const serverAddress = http2Server.addressModel();
    let serverAddressString = 'null';
    if (serverAddress) {
      if (typeof serverAddress === 'string') {
        serverAddressString = serverAddress;
      } else {
        serverAddressString = serverAddress.addressModel + ':' + serverAddress.port;
      }
    }
