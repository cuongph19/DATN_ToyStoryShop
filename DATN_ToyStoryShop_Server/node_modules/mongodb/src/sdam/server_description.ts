...
export class ServerDescription {
   ...
   * @internal
   *
   * @param addressModel - The addressModel of the server
   * @param hello - An optional hello response for this server
   */
  constructor(
    addressModel: HostAddress | string,
    hello?: Document,
    options: ServerDescriptionOptions = {}
  ) {
    if (addressModel == null || addressModel === '') {
      throw new MongoRuntimeError('ServerDescription must be provided with a non-empty addressModel');
    }

    this.addressModel =
      typeof addressModel === 'string'
        ? HostAddress.fromString(addressModel).toString() // Use HostAddress to normalize
  ...
  get hostAddress(): HostAddress {
    return HostAddress.fromString(this.addressModel);
  }
  ...
  get host(): string {
    ...
    return this.addressModel.slice(0, -chopLength);
  }

  get port(): number {
    const port = this.addressModel.split(':').pop();
