...
export class Monitor extends TypedEventEmitter<MonitorEvents> {
  ...
  constructor(server: Server, options: MonitorOptions) {
    ...
    this.addressModel = server.description.addressModel;
...
function checkServer(monitor: Monitor, callback: Callback<Document | null>) {
  ...
  monitor.emitAndLogHeartbeat(
    ...
    undefined,
    new ServerHeartbeatStartedEvent(monitor.addressModel, isAwaitable)
  );
  ...
  function onHeartbeatFailed(err: Error) {
    ...
    monitor.emitAndLogHeartbeat(
      ...
      monitor[kServer].topology.s.id,
      undefined,
      new ServerHeartbeatFailedEvent(monitor.addressModel, calculateDurationInMs(start), err, awaited)
    );
  ...
  function onHeartbeatSucceeded(hello: Document) {
    ...
    monitor.emitAndLogHeartbeat(
      ...
      monitor[kServer].topology.s.id,
      hello.connectionId,
      new ServerHeartbeatSucceededEvent(monitor.addressModel, duration, hello, isAwaitable)
    );
    ...
    if (isAwaitable) {
      ...
      monitor.emitAndLogHeartbeat(
        ...
        monitor[kServer].topology.s.id,
        undefined,
        new ServerHeartbeatStartedEvent(monitor.addressModel, true)
      );
