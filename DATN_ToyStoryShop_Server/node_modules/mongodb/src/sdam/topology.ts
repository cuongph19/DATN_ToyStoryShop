...
export interface TopologyPrivate {
  ...
  /** A map of server instances to normalized addressModels */
...
export class Topology extends TypedEventEmitter<TopologyEvents> {
      ...
      if (
        ...
        this.client.mongoLogger?.debug(
          ...
          new ServerSelectionSucceededEvent(
            selector,
            this.description,
            transaction.server.pool.addressModel,
  ...
  serverUpdateHandler(serverDescription: ServerDescription): void {
    if (!this.s.description.hasServer(serverDescription.addressModel)) {
      return;
    }
    ...
    const previousTopologyDescription = this.s.description;
    const previousServerDescription = this.s.description.servers.get(serverDescription.addressModel);
    ...
    if (!equalDescriptions) {
      const newDescription = this.s.description.servers.get(serverDescription.addressModel);
      if (newDescription) {
        this.emit(
...
function destroyServer(server: Server, topology: Topology) {
  ...
  topology.emitAndLog(
    ...
    new ServerClosedEvent(topology.s.id, server.description.addressModel)
  );
...
function createAndConnectServer(topology: Topology, serverDescription: ServerDescription) {
  topology.emitAndLog(
    ...
    new ServerOpeningEvent(topology.s.id, serverDescription.addressModel)
  );
...
function updateServers(topology: Topology, incomingServerDescription?: ServerDescription) {
  ...
  if (incomingServerDescription && topology.s.servers.has(incomingServerDescription.addressModel)) {
    const server = topology.s.servers.get(incomingServerDescription.addressModel);
    if (server) {
  ...
  for (const serverDescription of topology.description.servers.values()) {
    if (!topology.s.servers.has(serverDescription.addressModel)) {
      const server = createAndConnectServer(topology, serverDescription);
      topology.s.servers.set(serverDescription.addressModel, server);
    }
  }
...
function processWaitQueue(topology: Topology) {
  ...
  for (let i = 0; i < membersToProcess; ++i) {
    ...
    if (selectedDescriptions.length === 0) {
      ...
      selectedServer = topology.s.servers.get(selectedDescriptions[0].addressModel);
    } else {
      const descriptions = shuffle(selectedDescriptions, 2);
      const server1 = topology.s.servers.get(descriptions[0].addressModel);
      const server2 = topology.s.servers.get(descriptions[1].addressModel);
    ...
    if (
      ...
      topology.client.mongoLogger?.debug(
        ...
        new ServerSelectionSucceededEvent(
          waitQueueMember.serverSelector,
          waitQueueMember.topologyDescription,
          selectedServer.pool.addressModel,
          waitQueueMember.operationName
        )
      );
    }
...
function isStaleServerDescription(
  ...
  const currentServerDescription = topologyDescription.servers.get(
    incomingServerDescription.addressModel
  );
