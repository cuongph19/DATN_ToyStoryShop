...
export class TopologyDescription {
  ...
  constructor(
    ...
    for (const serverDescription of this.servers.values()) {
      ...
      if (serverDescription.minWireVersion > MAX_SUPPORTED_WIRE_VERSION) {
        ...
        this.compatibilityError = `Server at ${serverDescription.addressModel} requires wire version ${serverDescription.minWireVersion}, but this version of the driver only supports up to ${MAX_SUPPORTED_WIRE_VERSION} (MongoDB ${MAX_SUPPORTED_SERVER_VERSION})`;
      }

      if (serverDescription.maxWireVersion < MIN_SUPPORTED_WIRE_VERSION) {
        this.compatible = false;
        this.compatibilityError = `Server at ${serverDescription.addressModel} reports wire version ${serverDescription.maxWireVersion}, but this version of the driver requires at least ${MIN_SUPPORTED_WIRE_VERSION} (MongoDB ${MIN_SUPPORTED_SERVER_VERSION}).`;
        break;
      }
    }
  ...
  updateFromSrvPollingEvent(ev: SrvPollingEvent, srvMaxHosts = 0): TopologyDescription {
    /** The SRV addressModels defines the set of addressModels we should be using */
  ...
  update(serverDescription: ServerDescription): TopologyDescription {
    const addressModel = serverDescription.addressModel;
    ...
    if (
      ...
      if (topologyType === TopologyType.Single) {
        ...
        serverDescription = new ServerDescription(addressModel);
      } else {
        serverDescriptions.delete(addressModel);
      }
    }

    // update the actual server description
    serverDescriptions.set(addressModel, serverDescription);
  ...
  /**
   * Determines if the topology has a definition for the provided addressModel
   * @internal
   */
  hasServer(addressModel: string): boolean {
    return this.servers.has(addressModel);
  }
...
function updateRsFromPrimary(
  ...
  if (setName !== serverDescription.setName) {
    serverDescriptions.delete(serverDescription.addressModel);
  ...
  if (serverDescription.maxWireVersion >= 17) {
    ...
    if (maxElectionIdIsLess || (maxElectionIdIsEqual && maxSetVersionIsLessOrEqual)) {
      ...
      serverDescriptions.set(
        serverDescription.addressModel,
        new ServerDescription(serverDescription.addressModel)
      );
  ...
  for (const [addressModel, server] of serverDescriptions) {
    if (server.type === ServerType.RSPrimary && server.addressModel !== serverDescription.addressModel) {
      // Reset old primary's type to Unknown.
      serverDescriptions.set(addressModel, new ServerDescription(server.addressModel));
  ...
  serverDescription.allHosts.forEach((addressModel: string) => {
    if (!serverDescriptions.has(addressModel)) {
      serverDescriptions.set(addressModel, new ServerDescription(addressModel));
    }
  });
...
function updateRsWithPrimaryFromMember(
  ...
  if (
    ...
    (serverDescription.me && serverDescription.addressModel !== serverDescription.me)
  ) {
    serverDescriptions.delete(serverDescription.addressModel);
  }
...
function updateRsNoPrimaryFromMember(
  ...
  if (serverDescription.me && serverDescription.addressModel !== serverDescription.me) {
    serverDescriptions.delete(serverDescription.addressModel);
  }
