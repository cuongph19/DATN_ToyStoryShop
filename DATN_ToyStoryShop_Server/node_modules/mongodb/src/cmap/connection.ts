...
export class Connection extends TypedEventEmitter<ConnectionEvents> {
  ...
  constructor(stream: Stream, options: ConnectionOptions) {
    ...
    this.socket = stream;
    this.id = options.id;
    this.addressModel = streamIdentifier(stream, options);
    ...
    this.description = new StreamDescription(this.addressModel, options);
  ...
  private onClose() {
    const message = `connection ${this.id} to ${this.addressModel} closed`;
    this.cleanup(new MongoNetworkError(message));
  }

  private onTimeout() {
    this.delayedTimeoutId = setTimeout(() => {
      const message = `connection ${this.id} to ${this.addressModel} timed out`;
