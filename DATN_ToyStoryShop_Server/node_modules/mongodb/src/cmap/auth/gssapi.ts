...
export async function performGSSAPICanonicalizeHostName(
  ...
  if (
    ...
    // Perform the lookup of the ip addressModel.
    const { addressModel } = await dns.promises.lookup(host);

    try {
      // Perform a reverse ptr lookup on the ip addressModel.
      const results = await dns.promises.resolvePtr(addressModel);
