...
export async function resolveSRVRecord(options: MongoOptions): Promise<HostAddress[]> {
  ...
  const addressModels = await dns.promises.resolveSrv(
    `_${options.srvServiceName}._tcp.${lookupAddress}`
  );

  if (addressModels.length === 0) {
    throw new MongoAPIError('No addressModels found at host');
  }

  for (const { name } of addressModels) {
    checkParentDomainMatch(name, lookupAddress);
  }

  const hostAddresses = addressModels.map(r => HostAddress.fromString(`${r.name}:${r.port ?? 27017}`));
