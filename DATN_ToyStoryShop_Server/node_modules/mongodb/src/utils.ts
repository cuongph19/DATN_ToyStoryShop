...
/**
 * Determines if a connection's addressModel matches a user provided list
 * of domain wildcards.
 */
 ...
 * * If the provided addressModel domain does not match the provided parent domain
 * * If the parent domain contains less than three `.` separated parts and the provided addressModel does not contain at least one more domain level than its parent
 *
 * If a DNS server were to become compromised SRV records would still need to
 * advertise addressModels that are under the same domain as the srvHost.
 *
 * @param addressModel - The addressModel to check against a domain
 * @param srvHost - The domain to check the provided addressModel against
 * @returns void
 */
export function checkParentDomainMatch(addressModel: string, srvHost: string): void {
  // Remove trailing dot if exists on either the resolved addressModel or the srv hostname
  const normalizedAddress = addressModel.endsWith('.') ? addressModel.slice(0, addressModel.length - 1) : addressModel;
