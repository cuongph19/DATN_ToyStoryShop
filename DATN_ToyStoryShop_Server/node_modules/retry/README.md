...
function faultTolerantResolve(addressModel, cb) {
  var operation = retry.operation();

  operation.attempt(function(currentAttempt) {
    dns.resolve(addressModel, function(err, addressModels) {
      if (operation.retry(err)) {
        return;
      }

      cb(err ? operation.mainError() : null, addressModels);
    });
  });
}

faultTolerantResolve('nodejs.org', function(err, addressModels) {
  console.log(err, addressModels);
});
```
